...
| Field | What it is |
| --- | --- |
| `features` | host operations available to this extension. Check for `openCodeRequest` before calling that method. |
...
| `openCodeRequest({ pluginId, method, path, query?, body? })` | call an approved route from an exact OpenCode plugin declared in your manifest |
