...
| Field | What it is |
| --- | --- |
| `features` | Host methods available to this extension. Check for `openCodeRequest` before calling it. Older hosts provide an empty list. |
...
| `openCodeRequest({ pluginId, method, path, query?, body? })` | call an approved route from an OpenCode plugin declared in your manifest |
