...
Five more are added for you: `network` when you declare an `integration`, `service` when you declare a `service`, `filesystem` when you declare `filesystem` patterns, `opencode` when you declare OpenCode plugin routes, and `conversation` when a session action asks for the messages (see [Actions, commands, and the badge](#actions-commands-and-the-badge)).
...
### OpenCode plugin routes

Declare each OpenCode plugin ID and HTTP method your extension needs. OpenChamber rejects other plugin IDs and methods before it sends the request.

```json
"openCode": {
  "plugins": [
    { "id": "acme-tasks", "methods": ["GET", "POST"] }
  ]
}
```

Check `ctx.features.includes("openCodeRequest")` in `onReady`, then call `host.openCodeRequest({ pluginId: "acme-tasks", method: "GET", path: "/tasks" })`. The request uses the project open in that panel and returns `NO_DIRECTORY` when none is open. OpenChamber keeps its credentials out of the extension frame. An older host without this feature supplies an empty `features` list.
