...
Declare each OpenCode plugin ID and HTTP method the extension needs. OpenChamber rejects undeclared plugins and methods before forwarding a request.
...
Call the plugin with `host.openCodeRequest({ pluginId: "acme-tasks", method: "GET", path: "/tasks" })`. The request goes through the selected OpenCode runtime and never exposes its credentials to the iframe. Check `ctx.features.includes("openCodeRequest")` first. A host that omits the feature does not implement this transport.
