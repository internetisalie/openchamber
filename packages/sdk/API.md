...
| Field          | Type                     | Meaning                                                                                  |
| -------------- | ------------------------ | ---------------------------------------------------------------------------------------- |
| `features` | `HostFeature[]` | Host methods available to this extension. Check for `openCodeRequest` before calling it. Older hosts yield `[]`. |
...
`request` **/** `serviceRequest` **/** `openCodeRequest` **rules:** `method` is `GET` | `POST` | `PUT` | `PATCH` | `DELETE`. `path` must start with `/`, contain no scheme or traversal, and stay under the host-selected origin. `openCodeRequest` additionally requires an exact `contributes.openCode.plugins` ID/method declaration and the scoped `opencode` grant. It targets the project directory open when the call runs and returns `NO_DIRECTORY` if none is open. It accepts no headers; OpenCode and OpenChamber credentials remain in the host. Its response body is capped at 4 MiB; the other request APIs keep their 256,000-character cap.
